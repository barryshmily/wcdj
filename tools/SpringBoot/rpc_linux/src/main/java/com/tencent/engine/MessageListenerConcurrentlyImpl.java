...
public class MessageListenerConcurrentlyImpl implements MessageListenerConcurrently {
	...
	public ConsumeConcurrentlyStatus consumeMessage(
		...
		log.info(Thread.currentThread().getName() + " receive new messages size[" + msgs.size() + "]");
		
		for (int i = 0; i < msgs.size(); i++) {
		
			MessageExt msg = msgs.get(i);
			String body = new String(msg.getBody());
			log.info("msg " + i + " [" + body + "] ReconsumeTimes[" + msg.getReconsumeTimes() + "]");
			
			int ret = processMQRequest(body);
			log.info("ret[" + ret + "]");
		}
	...
	private int processMQRequest(String request) {
		...
		log.info("app_header[" + app_header +
				"] mq_header[" + mq_header +
				"] content[" + content +"]");
		...
		ResultDao res = null;
		try {
			res = TransMachine.instance().onMQRequest(uuid, trans_name, content);
		...
		if (res.getOk() == 0) {
			log.info("response uuid[" + uuid + "] transname[" + trans_name + "] ok, rsp[" + res.getMsg() + "]");
		} else {
			log.info("response uuid[" + uuid + "] transname[" + trans_name + "] err, rsp[" + res.getMsg() + "]");
		}
