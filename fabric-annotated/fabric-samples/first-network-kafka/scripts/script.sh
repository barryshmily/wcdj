...
checkOSNAvailability() {
	...
	do
		 ...
		 if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
			 peer channel fetch 0 -o orderer0.example.com:7050 -c "testchainid" >&log.txt
		 else
			 peer channel fetch 0 0_block.pb -o orderer0.example.com:7050 -c "testchainid" --tls --cafile $ORDERER_CA >&log.txt
		 fi
