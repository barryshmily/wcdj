...
function networkDown () {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts2/*.block channel-artifacts2/*.tx crypto-config
...
# artifacts, and subsequently output these files into the ``channel-artifacts2``
# folder.
#
...
function generateChannelArtifacts() {
  ...
  configtxgen -profile TwoOrgsOrdererGenesis -outputBlock ./channel-artifacts2/genesis.block
  if [ "$?" -ne 0 ]; then
  ...
  echo "#################################################################"
  configtxgen -profile TwoOrgsChannel -outputCreateChannelTx ./channel-artifacts2/channel.tx -channelID $CHANNEL_NAME
  if [ "$?" -ne 0 ]; then
  ...
  echo "#################################################################"
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts2/Org1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org1MSP
  if [ "$?" -ne 0 ]; then
  ...
  ./channel-artifacts2/Org2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org2MSP
  if [ "$?" -ne 0 ]; then
