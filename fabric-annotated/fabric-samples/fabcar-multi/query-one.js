...
Fabric_Client.newDefaultKeyValueStore({ path: store_path
	...
	const request = {
		...
		//targets: [peer0, peer1, peer2, peer3],
		chaincodeId: 'fabcar',
		fcn: 'queryAllCars',
		args: ['']
	};
	...
	if (query_responses && query_responses.length >= 1) {
		...
		for (let i in query_responses) {
			if (query_responses[i] instanceof Error) {
				console.log("error from query = ", query_responses[i]);	
			} else {
				console.log("Response is ", query_responses[i].toString());
			}
		}
	} else {
