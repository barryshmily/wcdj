...
var defaults = TopLevel{
	General: General{
		...
		LogLevel:    "DEBUG",
		LogFormat:   "%{color}%{time:2006-01-02 15:04:05.000 MST} [%{module}] %{longfile} %{shortfunc} -> %{level:.4s} %{id:03x}%{color:reset} %{message}",
		LocalMSPDir: "msp",
		LocalMSPID:  "DEFAULT",
...
func Load() *TopLevel {
	...
	logger.Debugf("[gerry] configName[%s]", configName)
