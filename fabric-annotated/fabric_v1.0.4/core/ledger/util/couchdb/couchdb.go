			...
			default:
				...
				if strings.TrimSpace(contentDispositionParts[0]) == "attachment" {
					...
					case "gzip": //See if the part is gzip encoded
						...
						logger.Debugf("[gerry] Retrieved attachment data[%v]", respBody)
					...
					default:
						...
						logger.Debugf("[gerry] Retrieved attachment data[%v]", partdata)
...
func (dbclient *CouchDatabase) QueryDocuments(query string) (*[]QueryResult, error) {
	...
	logger.Debugf("[gerry] Exiting QueryDocuments() results[%v]", results)
...
func (couchInstance *CouchInstance) handleRequest(method, connectURL string, data []byte, rev string,
	...
	logger.Debugf("[gerry] Entering handleRequest()  method=%s  url=%v data=%v rev=%v multipartB=%v maxRetries=%v keepConnectionOpen=%v", 
	      method, connectURL, data, rev, multipartBoundary, maxRetries, keepConnectionOpen)
