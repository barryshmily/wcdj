...
func (e *Endorser) endorseProposal(ctx context.Context, chainID string, txid string, signedProp *pb.SignedProposal, proposal *pb.Proposal, response *pb.Response, simRes []byte, event *pb.ChaincodeEvent, visibility []byte, ccid *pb.ChaincodeID, txsim ledger.TxSimulator, cd *ccprovider.ChaincodeData) (*pb.ProposalResponse, error) {
	...
	endorserLogger.Debugf("[gerry] *ccprovider.ChaincodeData: %v", cd)
...
func (e *Endorser) ProcessProposal(ctx context.Context, signedProp *pb.SignedProposal) (*pb.ProposalResponse, error) {
	...
	if err != nil {
		endorserLogger.Errorf("[gerry] validation.ValidateProposalMessage err:%v", err.Error())
		return &pb.ProposalResponse{Response: &pb.Response{Status: 500, Message: err.Error()}}, err
	}

	chdr, err := putils.UnmarshalChannelHeader(hdr.ChannelHeader)
	if err != nil {
		endorserLogger.Errorf("[gerry] putils.UnmarshalChannelHeader err:%v", err.Error())
		return &pb.ProposalResponse{Response: &pb.Response{Status: 500, Message: err.Error()}}, err
	}

	shdr, err := putils.GetSignatureHeader(hdr.SignatureHeader)
	if err != nil {
		endorserLogger.Errorf("[gerry] putils.GetSignatureHeader err:%v", err.Error())
		return &pb.ProposalResponse{Response: &pb.Response{Status: 500, Message: err.Error()}}, err
	}
	...
	if chainID != "" {
		endorserLogger.Debugf("[gerry] chainID: %s", chainID)
		// here we handle uniqueness check and ACLs for proposals targeting a chain
		lgr := peer.GetLedger(chainID)
		if lgr == nil {
			endorserLogger.Errorf("[gerry] peer.GetLedger err, chainID: %s", chainID)
			return nil, fmt.Errorf("failure while looking up the ledger %s", chainID)
		}
		if _, err := lgr.GetTransactionByID(txid); err == nil {
			endorserLogger.Errorf("[gerry] lgr.GetTransactionByID(txid: %s) err: %v", txid, err.Error())
	...
	endorserLogger.Debugf("[gerry] to get TxSimulator and HistoryQueryExecutor")
	...
	if chainID != "" {
		if txsim, err = e.getTxSimulator(chainID); err != nil {
			endorserLogger.Errorf("[gerry] e.getTxSimulator(chainID: %s) err: %v", chainID, err.Error())
			return &pb.ProposalResponse{Response: &pb.Response{Status: 500, Message: err.Error()}}, err
		}
		if historyQueryExecutor, err = e.getHistoryQueryExecutor(chainID); err != nil {
			endorserLogger.Errorf("[gerry] e.getHistoryQueryExecutor(chainID: %s) err: %v", chainID, err.Error())
			return &pb.ProposalResponse{Response: &pb.Response{Status: 500, Message: err.Error()}}, err
		}
	...
	//1 -- simulate
	endorserLogger.Debugf("[gerry] 1 -- simulate")
	cd, res, simulationResult, ccevent, err := e.simulateProposal(ctx, chainID, txid, signedProp, prop, hdrExt.ChaincodeId, txsim)
	if err != nil {
		endorserLogger.Errorf("[gerry] e.simulateProposal err: %v", err.Error())
		return &pb.ProposalResponse{Response: &pb.Response{Status: 500, Message: err.Error()}}, err
	}
	if res != nil {
		if res.Status >= shim.ERROR {
			...
			if ccevent != nil {
				...
				if err != nil {
					endorserLogger.Errorf("[gerry] GetBytesChaincodeEvent err: %v", err.Error())
					return nil, fmt.Errorf("failed to marshal event bytes - %s", err)
				}
			}
			pResp, err := putils.CreateProposalResponseFailure(prop.Header, prop.Payload, res, simulationResult, cceventBytes, hdrExt.ChaincodeId, hdrExt.PayloadVisibility)
			if err != nil {
				endorserLogger.Errorf("[gerry] CreateProposalResponseFailure err: %v", err.Error())
				return &pb.ProposalResponse{Response: &pb.Response{Status: 500, Message: err.Error()}}, err
			}
	...
	endorserLogger.Debugf("[gerry] 2 -- endorse and get a marshalled ProposalResponse message")
	var pResp *pb.ProposalResponse

	//TODO till we implement global ESCC, CSCC for system chaincodes
	//chainless proposals (such as CSCC) don't have to be endorsed
	if chainID == "" {
		endorserLogger.Debugf("chainID is NULL, txid: %s", txid)
		pResp = &pb.ProposalResponse{Response: res}
		
	} else {
		pResp, err = e.endorseProposal(ctx, chainID, txid, signedProp, prop, res, simulationResult, ccevent, hdrExt.PayloadVisibility, hdrExt.ChaincodeId, txsim, cd)
		if err != nil {
			endorserLogger.Errorf("[gerry] e.endorseProposal err: %v", err.Error())
			return &pb.ProposalResponse{Response: &pb.Response{Status: 500, Message: err.Error()}}, err
		}
		if pResp != nil {
			if res.Status >= shim.ERRORTHRESHOLD {
    ...
    endorserLogger.Debugf("go here txid: %s", txid)
