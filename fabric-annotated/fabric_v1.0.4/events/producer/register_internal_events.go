...
func getMessageType(e *pb.Event) pb.EventType {
	...
	case *pb.Event_Register:
	    logger.Debugf("[gerry] Event_Register")
		return pb.EventType_REGISTER
	case *pb.Event_Block:
	    logger.Debugf("[gerry] Event_Block")
		return pb.EventType_BLOCK
	case *pb.Event_ChaincodeEvent:
	    logger.Debugf("[gerry] Event_ChaincodeEvent")
		return pb.EventType_CHAINCODE
	case *pb.Event_Rejection:
	    logger.Debugf("[gerry] EventType_CHAINCODE")
		return pb.EventType_REJECTION
	default:
	    logger.Debugf("[gerry] invalid EventType")
		return -1
	}
}
