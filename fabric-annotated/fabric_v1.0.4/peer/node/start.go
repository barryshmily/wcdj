...
func serve(args []string) error {
	...
	logger.Debug("[gerry] before ledgermgmt.Initialize")
	ledgermgmt.Initialize()
	logger.Debug("[gerry] after ledgermgmt.Initialize")
	
	// Parameter overrides must be processed before any parameters are
	// cached. Failures to cache cause the server to terminate immediately.
	if chaincodeDevMode {
		logger.Debug("[gerry] chaincodeDevMode");
	...
	logger.Debug("[gerry] before peer.CacheConfiguration")
	if err := peer.CacheConfiguration(); err != nil {
		return err
	}
	logger.Debug("[gerry] after peer.CacheConfiguration")

	logger.Debug("[gerry] before peer.GetPeerEndpoint")
	peerEndpoint, err := peer.GetPeerEndpoint()
	if err != nil {
		err = fmt.Errorf("Failed to get Peer Endpoint: %s", err)
		return err
	}
	logger.Debug("[gerry] after peer.GetPeerEndpoint")

	listenAddr := viper.GetString("peer.listenAddress")

	logger.Debug("[gerry] before peer.GetSecureConfig")
	secureConfig, err := peer.GetSecureConfig()
	if err != nil {
		logger.Fatalf("Error loading secure config for peer (%s)", err)
	}
	logger.Debug("[gerry] after peer.GetSecureConfig")
	
	logger.Debug("[gerry] before peer.CreatePeerServer")
	peerServer, err := peer.CreatePeerServer(listenAddr, secureConfig)
	if err != nil {
		logger.Fatalf("Failed to create peer server (%s)", err)
	}
	logger.Debug("[gerry] after peer.CreatePeerServer")
	...
	// TODO gerry
	logger.Debug("[gerry] before createEventHubServer")
	//TODO - do we need different SSL material for events ?
	ehubGrpcServer, err := createEventHubServer(secureConfig)  // 内部创建一个EventHub协程
	if err != nil {
		grpclog.Fatalf("Failed to create ehub server: %v", err)
	}
	logger.Debug("[gerry] after createEventHubServer")

	// enable the cache of chaincode info
	ccprovider.EnableCCInfoCache()


	logger.Debug("[gerry] before createChaincodeServer")
	ccSrv, ccEpFunc := createChaincodeServer(peerServer, listenAddr)
	logger.Debug("[gerry] after createChaincodeServer")
	
	logger.Debug("[gerry] before registerChaincodeSupport")
	registerChaincodeSupport(ccSrv.Server(), ccEpFunc)
	logger.Debug("[gerry] after registerChaincodeSupport")
	
	logger.Debug("[gerry] before go ccSrv.Start()")
	go ccSrv.Start()                                 // 创建一个ccSrv的协程
	logger.Debug("[gerry] after go ccSrv.Start()")

	logger.Debugf("Running peer")

	// Register the Admin server
	logger.Debug("[gerry] before RegisterAdminServer")
	pb.RegisterAdminServer(peerServer.Server(), core.NewAdminServer())
	logger.Debug("[gerry] after RegisterAdminServer")

	// Register the Endorser server
	logger.Debug("[gerry] before RegisterEndorserServer")
	serverEndorser := endorser.NewEndorserServer()
	pb.RegisterEndorserServer(peerServer.Server(), serverEndorser)
	logger.Debug("[gerry] after RegisterEndorserServer")

	// Initialize gossip component
	logger.Debug("[gerry] before Initialize gossip component")
	...
	err = service.InitGossipService(serializedIdentity, peerEndpoint.Address, peerServer.Server(),
		messageCryptoService, secAdv, secureDialOpts, bootstrap...)  // 内部创建一个gossip的协程
	if err != nil {
		return err
	}
	defer service.GetGossipService().Stop()
	logger.Debug("[gerry] after Initialize gossip component")

	//initialize system chaincodes
	logger.Debug("[gerry] before initSysCCs")
	initSysCCs()
	logger.Debug("[gerry] after initSysCCs")

	//this brings up all the chains (including testchainid)
	logger.Debug("[gerry] before peer.Initialize")
	peer.Initialize(func(cid string) {
		logger.Debugf("Deploying system CC, for chain <%s>", cid)
		scc.DeploySysCCs(cid)
	})
	logger.Debug("[gerry] after peer.Initialize")
