...
func chaincodeInvokeOrQuery(cmd *cobra.Command, args []string, invoke bool, cf *ChaincodeCmdFactory) (err error) {
	...
	if invoke {
		if proposalResp.Response.Status >= shim.ERROR {
			...
			logger.Infof("Chaincode invoke successful. result: %v", ca.Response)// 返回格式：status:200
		}
	} else {
		if proposalResp == nil {
		...
		logger.Debugf("[gerry] Query Result, proposalResp.Response.Payload[x]: %x\n", proposalResp.Response.Payload)
		logger.Debugf("[gerry] Query Result, proposalResp.Response.Payload[s]: %s\n", string(proposalResp.Response.Payload))
	}
	return nil
}
